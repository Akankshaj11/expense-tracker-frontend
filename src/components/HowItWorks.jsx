  ...
  return (
    <section className="mt-16 scroll-mt-28">
      <div className="max-w-2xl">
        <p className="text-sm font-semibold uppercase tracking-[0.22em] text-primary-600">How it works</p>
        <h3 className="mt-3 text-3xl font-bold tracking-tight text-[var(--text)] sm:text-4xl">A simple three-step workflow</h3>
      </div>
        ...
        {steps.map((s,i)=> (
          <motion.article key={s.title} initial={{opacity:0,y:14}} whileInView={{opacity:1,y:0}} viewport={{ once:true, amount:0.35 }} transition={{delay:0.08*i, duration:0.45}} className="card-floating relative rounded-[1.75rem] p-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-semibold text-primary-700">0{i + 1}</span>
            </div>
            <div className="mt-5 flex items-center gap-3 text-primary-200">
              {Array.from({ length: i < 2 ? 2 : 0 }).map((_, idx) => <span key={idx} className="h-px flex-1 bg-primary-100" />)}
            </div>
            <h4 className="mt-5 text-xl font-semibold text-[var(--text)]">{s.title}</h4>
