      ...
      >
        <div className="w-full rounded-[1.5rem] border border-white/80 bg-white p-4 shadow-2xl shadow-primary-500/10 sm:p-6">
          ...
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h1 className="mt-1.5 text-2xl font-light tracking-tight text-slate-900 sm:text-3xl">Select Your Language</h1>
            <p className="mt-1 text-sm text-slate-600">Choose the UI language for your workspace.</p>
          </div>
            ...
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
              {languages.map((item) => (
                <button
                  ...
                  className={`rounded-xl border p-2.5 text-center transition hover:-translate-y-0.5 hover:shadow-md ${
                    language === item.code ? 'border-primary-500 bg-primary-50 shadow-md shadow-primary-500/10' : 'border-slate-200 bg-white'
                  }`}
                >
                  <div>
                    <p className="text-base font-light text-slate-900">{item.label}</p>
                    <p className="text-xs text-slate-600">{item.code.toUpperCase()}</p>
                  </div>
                </button>
              ))}
            </div>
